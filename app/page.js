...
import { MapPin, Calendar, DollarSign, Hotel, Bus, Star, TrendingUp, Users, Package } from 'lucide-react';
...
export default function App() {
  ...
  const handleUserSubmit = async () => {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        setCurrentUser({ UserID: data.data.UserID, Name: bookingForm.name, Email: bookingForm.email });
        setBookingStep(2);
      }
    } catch (error) {
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="packages" className="space-y-6">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-2">Explore Amazing Destinations</h2>
              <p className="text-gray-600">Discover your next adventure with our curated travel packages</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {packages.map((pkg) => (
                <Card key={pkg.PackageID} className="overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <CardHeader>
                    ...
                    <CardDescription className="line-clamp-2">{pkg.Description || 'Explore amazing destinations'}</CardDescription>
                  </CardHeader>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {pkg.destinations.slice(0, 3).map((dest) => (
                              <Badge key={dest.DestinationID} variant="outline" className="text-xs">
                                {dest.City}
                              </Badge>
                            ))}
                            {pkg.destinations.length > 3 && (
                              <Badge variant="outline" className="text-xs">+{pkg.destinations.length - 3}</Badge>
                            )}
                          </div>
                        </div>
                      </div>
            ...
            {!currentUser ? (
              <Card className="text-center py-12">
                <CardContent>
                  ...
                  <p className="text-gray-600 mb-4">Please make a booking to view your trips</p>
                  <Button onClick={() => setActiveTab('packages')} className="bg-blue-600 hover:bg-blue-700">
                    Browse Packages
                  </Button>
                </CardContent>
              </Card>
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm">{booking.HotelName}, {booking.HotelCity}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Star className="h-4 w-4 text-yellow-500" />
                              <span className="text-sm">Rating: {booking.HotelRating}/5</span>
                            </div>
                          </div>
                          <div className="space-y-2">
                            <div className="flex items-center space-x-2">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{analytics.stats?.TotalBookings || 0}</div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-green-600">${analytics.stats?.TotalRevenue || 0}</div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">${analytics.stats?.AverageBookingValue?.toFixed(2) || 0}</div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-blue-600">{analytics.stats?.TotalUsers || 0}</div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {analytics.popularPackages?.slice(0, 5).map((pkg, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex-1">
                            <p className="font-medium text-gray-900">{pkg.Title}</p>
                            <p className="text-sm text-gray-600">{pkg.TotalBookings || 0} bookings</p>
                          </div>
                          <div className="text-right">
                            <p className="font-semibold text-green-600">${pkg.TotalRevenue || 0}</p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {analytics.bookingsPerDestination?.slice(0, 6).map((dest, index) => (
                        <div key={index} className="flex items-center space-x-3 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
                          ...
                          <Badge variant="secondary">{dest.TotalBookings}</Badge>
                        </div>
                      ))}
                    </div>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {analytics.revenuePerHotel?.slice(0, 5).map((hotel, index) => (
                        <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="text-right">
                              <p className="text-sm font-medium">{hotel.TotalBookings || 0} bookings</p>
                              <p className="text-xs text-gray-500">${hotel.TotalRevenue || 0} revenue</p>
                            </div>
                            <div className="flex items-center space-x-1">
                              <Star className="h-4 w-4 text-yellow-500" />
                              <span className="text-sm font-medium">{hotel.AverageRating?.toFixed(1) || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            <div className="space-y-4">
              <Card className="bg-blue-50">
                <CardContent className="pt-4">
                  <div className="space-y-2">
                    <p className="font-semibold text-lg">{selectedPackage?.Title}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">Duration: {selectedPackage?.Duration} days</span>
                      <span className="text-xl font-bold text-green-600">${selectedPackage?.Price}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setBookingForm({ ...bookingForm, hotelId: value })}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setBookingForm({ ...bookingForm, paymentMode: value })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
